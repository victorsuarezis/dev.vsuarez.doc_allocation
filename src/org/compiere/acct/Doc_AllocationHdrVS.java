 ...
 *  - Bug fixed: Null Pointer exceptions when Amount of Documents Lines is ZERO 
 *
 */
public class Doc_AllocationHdrVS extends Doc_AllocationHdr
{
	/**
	 *  Constructor
	 ...
	 * 	@param trxName trx
	 */
	public Doc_AllocationHdrVS (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, rs, trxName);
	}   //  Doc_Allocation
...
//					line.setC_ConversionType_ID(invoice.getC_ConversionType_ID());
		...
		MAccount DiscountAccount, MAccount WriteOffAccoint, boolean isSOTrx)
	{
		...
		Doc_AllocationTaxVS tax = new Doc_AllocationTaxVS (
		...
		MPayment payment, BigDecimal allocationSource, BigDecimal allocationAccounted)
	{
		...
		if (valuesPay != null) {
			...
			if ((paymentSource == null || paymentSource.signum() == 0) && (paymentAccounted == null || paymentAccounted.signum() == 0)) {
...
class Doc_AllocationTaxVS
{
	 ...
	 */
	public Doc_AllocationTaxVS (MAccount DiscountAccount, BigDecimal DiscountAmt,
	...
	private static final CLogger	log = CLogger.getCLogger(Doc_AllocationTaxVS.class);
